...
class LangChainFinanceAgent:
    ...
    def __init__(
        ...
        *,
        ...
        if not self.api_key:
            raise RuntimeError("OPENAI_API_KEY 未配置，无法使用LangChain Agent。")

        self._setup_agent()

    @staticmethod
    def _normalize_transactions(transactions: Iterable[Transaction | dict]) -> List[Transaction]:
    ...
    def _transactions_dataframe(self) -> pd.DataFrame:
        ...
        data = [
            {"date": pd.to_datetime(txn.date), "category": txn.category, "amount": float(txn.amount)}
    ...
    def _tool_query_budget(self, _: str) -> str:
        ...
        if self.monthly_budget <= 0:
            return "尚未设置月度预算。请先设定预算金额。"
        remaining = max(0.0, self.monthly_budget - spent)
        return f"月度预算：¥{self.monthly_budget:.2f}，已消费：¥{spent:.2f}，剩余：¥{remaining:.2f}。"

    def _tool_query_spending(self, _: str) -> str:
        totals = calculate_category_totals(self.transactions)
        if not totals:
            return "暂无消费数据。"
        lines = [f"{category}: ¥{amount:.2f}" for category, amount in sorted(totals.items(), key=lambda item: item[1], reverse=True)]
        return "各类别消费：\n" + "\n".join(lines[:5])
    ...
    def _tool_query_category(self, category: str) -> str:
        ...
        if amount is None:
            return f"未找到「{category}」类别的消费记录。"
        return f"「{category}」类别总支出约 ¥{amount:.2f}。"
    ...
    def _setup_agent(self) -> None:
        ...
        tools = [
            Tool(
                ...
                description="查询本月预算剩余等信息。",
            ),
            Tool(
                name="query_spending",
                func=self._tool_query_spending,
                description="获取各消费类别的汇总数据。",
            ),
            Tool(
                name="query_category",
                func=self._tool_query_category,
                description="输入消费类别名称，返回对应支出金额。",
            ),
        ]
    ...
    def run(self, query: str) -> str:
        ...
        try:
            ...
            raise RuntimeError("LangChain Agent 执行失败，请稍后重试。") from exc
